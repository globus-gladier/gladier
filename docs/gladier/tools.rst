...
Gladier Tools are the glue that holds together Globus Flows and FuncX functions. A tool bundles
everything that a FuncX function needs to run, so that the Gladier Client can register the function, 
...
A tool is composed of zero or more funcx_functions and a flow_definition. A flow can be specified
explicitly, or if it involves just a single FuncX function, can be generated automatically  by
